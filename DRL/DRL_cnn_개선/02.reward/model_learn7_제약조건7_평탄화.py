...
7번 논문 대비 우리가 바꾼 점(명시):
  (A) 무게(mass) 고려:
        - feasibility mask 에 CoM(무게중심) 마진 + heavy-on-light 규칙 추가.
...
class Config:
    ...
    # feasibility mask 파라미터
    k_hol = 1.8                       # heavy-on-light: 위 박스 mass <= k_hol * 아래 박스 mass
    com_alpha0 = 0.45                 # CoM 허용 오프셋(가벼운 박스), footprint 반치수 대비 비율
    com_alpha1 = 0.20                 # CoM 허용 오프셋(가장 무거운 박스) → 무거울수록 빡빡
    max_mass = 6.0                    # 정규화용 최대 질량

    # 보상
    phi = 100.0                        # 논문 스텝보상 스케일
    w_cog = 0.03                       # CoG 소프트 보상 가중치(우리 확장)
    alpha_hd = 3.0                    # 평탄화 보상 가중치 (9번 논문 r_HD 아이디어 도입)
                                      #  = (prev_h_std - cur_h_std) / Hc
                                      # 처음엔 0.2 로 시작하고 학습 결과 보고 0.1~0.5 조정
...
class PalletizingEnv:
    ...
    def _mask_one(self, box, rot):
        ...
        for di in range(lc):
            for dj in range(wc):
                ...
                # 지지하는 셀의 하부 박스 질량 중 최소(가장 약한 지지)
        ...
        # (3) 지지 기하(논문 60/80/95 + 모서리)
        geom = (((ratio > 0.60) & (corner_cnt >= 4)) |
                ((ratio > 0.80) & (corner_cnt >= 3)) |
                (ratio > 0.95))

        # (4) CoM 마진(무게로 스케일): 지지셀 중심이 footprint 중심에 가까울 것
        ...
        m = box["mass"] / cfg.max_mass
        alpha = cfg.com_alpha0 + (cfg.com_alpha1 - cfg.com_alpha0) * m   # 무거울수록 작아짐
        com_ok = (off_i <= alpha) & (off_j <= alpha)

        # (5) heavy-on-light: 바닥(base==0)이면 항상 OK, 아니면 mass <= k_hol*지지질량
        floor = (base == 0)
        safe_supp = np.where(np.isfinite(supp_mass_min), supp_mass_min, 0.0)
        hol_ok = floor | (box["mass"] <= cfg.k_hol * safe_supp)
    ...
    def step(self, action):
        ...
        # 평탄화 보상 (9번 논문 r_HD 아이디어): 이번 스텝에서 표면이 더 평평해질수록 +
        #   flatness_bonus = (prev_h_std - cur_h_std) / Hc   (정규화)
        #   첫 스텝은 prev_h_std=0 이라 값이 튀므로 0 으로 강제
        cur_h_std = float(self.height.astype(np.float32).std())
        if self.first_step:
            flatness_bonus = 0.0
            self.first_step = False
        else:
            flatness_bonus = (self.prev_h_std - cur_h_std) / cfg.Hc
        self.prev_h_std = cur_h_std

        reward = cfg.phi * (v_box - v_waste) + cog + cfg.alpha_hd * flatness_bonus
