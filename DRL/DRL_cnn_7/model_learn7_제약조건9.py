...
class PalletizingEnv:
    ...
    def _mask_one(self, box, rot):
        ...
        # (3) 지지 기하 (9번 논문):
        #     Σ 접촉 셀 수 >= 0.66 * footprint 면적.
        #     바닥(base == 0)이면 완전 지지로 간주 → 무조건 통과.
        floor = (base == 0)
        geom = floor | (support_cnt >= 0.66 * area)

        # (4) CoM 마진 → 제거

        # (5) heavy-on-light (9번 논문): 배치 박스 질량 <= 3 × 최소 지지 박스 질량
        #     바닥이면 통과, 아니면 3배 규칙 적용
        safe_supp = np.where(np.isfinite(supp_mass_min), supp_mass_min, 0.0)
        hol_ok = floor | (box["mass"] <= 3.0 * safe_supp)
